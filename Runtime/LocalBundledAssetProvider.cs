...
namespace UTJ
{
    ...
    /// </summary>
    [System.ComponentModel.DisplayName("Local-Asset Provider")]
    public class LocalBundledAssetProvider : ResourceProviderBase
    {
        internal class InternalOp
        {
            ...
            internal static T LoadBundleFromDependecies<T>(IList<object> results) where T : class, IAssetBundleResource
            {
                ...
                var firstBundleWrapper = true;
                for (var i = 0; i < results.Count; i++)
                {
                    if (results[i] is IAssetBundleResource abWrapper)
                    {
            ...
            public void Start(ProvideHandle provideHandle)
            {
                ...
                var deps = new List<object>(); // TODO: garbage. need to pass actual count and reuse the list
                ...
                else
                {
                    ...
                    // NOTE: I should throw feedback that it should be interface API
                    if (bundleResource is LocalAssetBundleResource assetBundleResource)
            ...
            private void BeginAssetLoad()
            {
                ...
                {
                    if (ResourceManagerConfig.ExtractKeyAndSubKey(assetPath, out var mainPath, out var subKey))
                    {
                        m_SubObjectName = subKey;
            ...
            private void ActionComplete(AsyncOperation obj)
            {
                if (m_RequestOperation != null)
                {
                    ...
                    else if (string.IsNullOrEmpty(m_SubObjectName))
            ...
            {
                return m_RequestOperation?.progress ?? 0.0f;
            }
        }
